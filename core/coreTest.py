...
if "big" in args.coreOff:
    disable_cores(1,1,1,1,0,0,0,0)
if "mid" in args.coreOff:
    disable_cores(1,1,1,1,0,0,1,1)
if "little" in args.coreOff:
    disable_cores(0,0,0,0,1,1,1,1)


turn_on_screen()
run_name = f"{int(time.time())}__default__{args.tempSet}__exp{args.experiment}__temp{args.temperature}__coreOff{args.coreOff}"
...
init_get_state(window)
sleep(initSleep)

c_states, temps, qos, t1a, t2a, littlea, mida, biga, gpua, a, gpu_util, freqs, _, _, _ = get_states2(window)
...
for i in range(total_timesteps):
    ...
    c_states, temps, qos, t1b, t2b, littleb, midb, bigb, gpub, b, gpu_util, freqs, _, _, _ = get_states2(window)
